...
---
## 通知 
- Atlasx到0.5版后将不再更新，存在过多bug，投入市场发现很多bug，后续将对0.5版进行修复后将不再维护，Pro版的获取密钥可以通过本项目里的脚本，感谢各位提出有效建议的师傅。
- 
- Atlasx 1.0版本已经在再重构中，增加很多新功能如：获取到子域名后对子域名的富化增加10源线索、llm高风险ai训练、通过输入1000份Web端的SRC漏洞报告等功能，后续将与企业合作开发更深度侧重于企业。
- 下图是是使用训练后的风险测试数据对之前收集的数据重新分析结果，经过测试新版本中高危风险存在漏洞率为76%。
- <img width="1272" height="630" alt="image" src="https://github.com/user-attachments/assets/f896ede6-d64c-4fc7-b761-923a394b4b03" />
...
**Pro 已公开**：平台签发私钥随仓库分发。复制设置页申请码 `AXID-…`，本机签发后粘贴激活：

```bash
pip install cryptography
python3 scripts/atlasx_public_pro_issue.py --mid 'AXID-XXXX-...'
# 把输出的 AX2-… 粘贴到 设置 → 系统 → Pro License
```

说明：同一私钥也用于升级包验签；公开后任何人可自签 Pro，也可伪造升级包——这是主动开放，请只装官方 Release。采集源不按许可阉割。
